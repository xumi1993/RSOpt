...
c        real*4 thkm(NLAY),vpm(NLAY),vsm(NLAY),rhom(NLAY)
        ...
        real(dp),INTENT(INOUT) :: cg(kmax)
        real(dp) :: twopi,one,onea
        real(dp) :: cc,c1,clow,cm,dc,t1
c       double precision t(NP),c(NP),cb(NP),cg(NP)
        real(dp) ::c(kmax),cb(kmax)
c       real*4 d(NL),a(NL),b(NL),rho(NL),rtp(NL),dtp(NL),btp(NL)
        real(cr) :: d(nlayer),a(nlayer),b(nlayer),rho(nlayer)
        real(cr ) :: rtp(nlayer),dtp(nlayer),btp(nlayer)
c        common/modl/ d,a,b,rho,rtp,dtp,btp     
c        common/para/ mmax,llw,twopi
        integer :: iverb(2),llw
        integer :: nsph, ifunc, idispl, idispr, is, ie
        real(cr):: sone0, ddc0, h0, sone, ddc, h
